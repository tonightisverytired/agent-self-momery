...
### 5.0 后续演进（2026-09-17 补记）

以上结论针对的是**只读检索后端**（`PostgresTimeBackend`，给时间链加速）。
2026-09-17 补做了另一条正交的**存储后端**轴：`PGStore` 让 PostgreSQL 可作为
**主存储**（读写全路径，与 `SQLiteStore` 同签名，配置开关切换，默认仍 SQLite）。

**这不否定本文结论**——小库（边 < 5 万）仍建议 SQLite，PG 主存储换来的是
多进程共享、服务化部署与运维能力，而不是延迟优势。落地方式、迁移与回退见
[Postgres后端使用手册](Postgres后端使用手册.md)。
