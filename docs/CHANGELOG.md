...
## 0.8.3（2026-09-21）— 写入侧时间链修复 + 人物页重设计

**写入侧时间（修复"对话写入不带时间"断链）**：
- `write_text/write_many` 自动注入 `meta["today"]`（调用方显式传入优先，离线注入可复现）——LLM 抽取器据此把「昨天/上周一」等相对时间换算成具体日期填入 ts；此前 `DeepSeekExtractor(today=None)` 时换算完全不生效
- 新增 `ChainedExtractor`（LLM 优先 + 失败/整批零产出退回 FallbackExtractor）：「教它东西」永远留得下记忆，API 故障不再"写了等于没写"
- `dnamemory-server` 默认接线：有 `DEEPSEEK_API_KEY` 自动启用抽取链，`--fallback-extractor` 仅用于离线/测试强制确定性；启动日志打印当前抽取器模式
- 时间路 `_time_path` 对 ts=NULL 的事件回退 created_at（写入时间锚点），回退命中分值减半（真实发生时间优先，防批量注入同一天 created_at 淹没时间路）；时间差按日历日计算（消除 .days 向下取整偏差）

**人物页重设计**（名片式，替代数据库表观感）：
- 左栏：头像（姓氏首字+按名定色）+ 同名合并（×N 徽章）+ 默认只列"有画像"的人（只被提过名字的空壳可勾选展开）
- 右栏：名片头（年龄/性别/职业等 chips + 统计）、画像按基本情况/工作与学业/生活与兴趣中文分组、列表值渲染为胶囊、关系区（`关系_*` 事实 + 相邻人物胶囊跳转）、经历按年份分组时间线（描述折叠展开）

**浏览口径**：`GET /facts?entity=X` 合并所有同名 active 人物节点的事实（基准库 1715 个人物节点仅 554 个名字；召回侧 fact_lookup 不受影响）
